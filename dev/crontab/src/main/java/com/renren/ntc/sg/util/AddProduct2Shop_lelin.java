...
public class AddProduct2Shop_lelin {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // 读取第一章表格内容
        AddProduct2Shop_lelin addProduct = new AddProduct2Shop_lelin();
        String filePath = "F:\\product\\乐邻货架条码及售价.txt";
            ...
            if(file.isFile() && file.exists()){ //判断文件是否存在
                ...
                while((lineTxt = bufferedReader.readLine()) != null){
                    ...
                    if (p != null )  {
                        ...
                        Item item = itemDao.getItem(SUtils.generTableName(shop_id),p.getSerialNo(),shop_id) ;
                         if(item == null ){
                             System.out.println( "insert " + it.getSerialNo() );
                          itemDao.insert(SUtils.generTableName(shop_id),it) ;
                         } else{
                             System.out.println( "update " + it.getSerialNo() );
                             itemDao.updatePrice(SUtils.generTableName(shop_id),item.getSerialNo(),price,shop_id );
                         }
                    }
                }
            }else{
