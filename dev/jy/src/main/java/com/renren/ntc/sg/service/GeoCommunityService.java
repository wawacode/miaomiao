...
import com.renren.ntc.sg.geo.GeoCommunityMongodbService2d;
...
@Service
public class GeoCommunityService {

    private static int DEFAULT_MAX_DISTANCE = 500 * 1000; // 50km
    private static int DEFAULT_MAX_NUM = 100;

    private static final Logger logger = Logger.getLogger(GeoCommunityService.class);


    private AbstractGeoMongodbService dbService;

    private GeoCommunityService() {
        dbService = new GeoCommunityMongodbService2d();
    }
    ...
    public static void addCommunity(Community c) {
        GeoCommunityService geo = new GeoCommunityService();
        ShopLocation shop = new ShopLocation();
        shop.setShop_id(c.getId());
        shop.setLongitude(c.getLng());
        shop.setLatitude(c.getLat());
        geo.updateLocation(shop);
