...
public class DaliyWxOrderDetailUtil {
	...
	public static void main(String[] args) {
		...
		MailSendServer mailSendServer = com.renren.ntc.sg.mail.MailSendServer.getServer("smtp.163.com", 25, "lee_yeah1@163.com", "Lee1qaz2wsx",
	              "lee_yeah1@163.com");
		  mailSendServer.sendTextInfo(new MailSendInfo("喵喵微信支付每日订单详情(每日的货到付款和每日用户点击确认订单的列表)",getHtmlInfo(wxpayShopReports,totalShopOrderPrice), mailLists, new String[0]),false);
		  //isRun = false;
	}
	
	private static String getHtmlInfo(List<WXPayBossShopReport> wxpPayShopReports,int totalShopOrderPrice){
	...
	for(WXPayBossShopReport wxPayShopReport : wxpPayShopReports){
		...
		if(loopCount == i){
			orderInfoHtml = orderInfoHtml + "<td nowrap> 订单时间</td> <td align='center'> 订单号</td> <td nowrap align='center'> 下单金额(元)</td> <td nowrap align='center'> 优惠券金额(元)</td> <td nowrap align='center'> 最终金额(元)</td><td nowrap align='center'>今日用户是否确认收货</td><td nowrap align='center'> 用户点击确认收货的时间</td><td nowrap align='center'> 客服点击退单</td><td nowrap rowspan='$1'>"+totalShopOrderPriceStr+"</td></tr>";
		}else {
			orderInfoHtml = orderInfoHtml + "<td nowrap> 订单时间</td> <td align='center'> 订单号</td> <td nowrap align='center'> 下单金额(元)</td> <td nowrap align='center'> 优惠券金额(元)</td> <td nowrap align='center'> 最终金额(元)</td><td nowrap align='center'>今日用户是否确认收货</td><td nowrap align='center'> 用户点击确认收货的时间</td><td nowrap align='center'> 客服点击退单</td></tr>";
		}
		
		for(WXPayDetail wxpayDetail : shopOrderFlows){
			if(!wxpayDetail.isKfCancel()){
				orderInfoHtml = orderInfoHtml + "<tr><td nowrap>"+wxpayDetail.getOrderTimeStr()+"</td><td align='center'>"+wxpayDetail.getOrderId()+"<td align='center'>"+wxpayDetail.getOrderPrice()+"</td> <td align='center'>"+wxpayDetail.getWxDiscount()+"</td> <td align='center'>"+wxpayDetail.getRealPrice()+"</td><td align='center'>"+(wxpayDetail.isTodayClickConfirm()?"是":"否")+"</td><td align='center'>"+wxpayDetail.getClickConfirmTime()+"</td><td align='center'>"+(wxpayDetail.isKfCancel()?"是":"否")+"</td></tr>";
			}else {
				orderInfoHtml = orderInfoHtml + "<tr bgcolor='red'><td nowrap>"+wxpayDetail.getOrderTimeStr()+"</td><td align='center'>"+wxpayDetail.getOrderId()+"<td align='center'>"+wxpayDetail.getOrderPrice()+"</td> <td align='center'>"+wxpayDetail.getWxDiscount()+"</td> <td align='center'>"+wxpayDetail.getRealPrice()+"</td><td align='center'>"+(wxpayDetail.isTodayClickConfirm()?"是":"否")+"</td><td align='center'>"+wxpayDetail.getClickConfirmTime()+"</td><td align='center'>"+(wxpayDetail.isKfCancel()?"是":"否")+"</td></tr>";
			}
